...
class WebhookRepository(BaseRepository[Webhook]):
    def __init__(self, session: AsyncSession) -> None:
        super().__init__(model=Webhook, session=session)  # type: ignore
    ...
    async def count_by_tenant(self, tenant_id: uuid.UUID) -> int:
        stmt = select(func.count()).select_from(Webhook).where(Webhook.tenant_id == tenant_id)
    ...
    async def increment_failure_count(self, webhook_id: uuid.UUID) -> None:
        stmt = (
            ...
            .values(failure_count=Webhook.failure_count + 1, last_failure_at=func.now())
        )
...
class WebhookDeliveryRepository(BaseRepository[WebhookDelivery]):
    def __init__(self, session: AsyncSession) -> None:
        super().__init__(model=WebhookDelivery, session=session)  # type: ignore

    async def get_by_webhook(
        self, webhook_id: uuid.UUID, offset: int = 0, limit: int = 50
    ...
    async def create_delivery(
        self, webhook_id: uuid.UUID, event_type: str, payload: dict[str, Any]
    ) -> WebhookDelivery:
