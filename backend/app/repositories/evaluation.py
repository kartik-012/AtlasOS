...
class EvaluationRunRepository(BaseRepository[EvaluationRun]):
    ...
    async def get_by_tenant(self, tenant_id: uuid.UUID, offset: int = 0, limit: int = 50) -> list[EvaluationRun]:
        stmt = select(EvaluationRun).where(
            EvaluationRun.tenant_id == tenant_id
        ).offset(offset).limit(limit).order_by(EvaluationRun.created_at.desc())
        
        result = await self._session.execute(stmt)
        return list(result.scalars().all())
        
    async def count_by_tenant(self, tenant_id: uuid.UUID) -> int:
        stmt = select(func.count()).select_from(EvaluationRun).where(
            EvaluationRun.tenant_id == tenant_id
        )
    ...
    async def create_run(self, tenant_id: uuid.UUID, run_type: str, triggered_by: uuid.UUID, config: dict[str, Any]) -> EvaluationRun:
        run = EvaluationRun(
            tenant_id=tenant_id,
            run_type=run_type,
            status="pending",
...
class EvaluationMetricRepository(BaseRepository[EvaluationMetric]):
    def __init__(self, session: AsyncSession) -> None:
        super().__init__(model=EvaluationMetric, session=session)
    ...
    async def create_metric(self, run_id: uuid.UUID, metric_name: str, metric_value: float, target_value: float, passed: bool, details: dict[str, Any]) -> EvaluationMetric:
