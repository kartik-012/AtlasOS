...
class ContradictionService:
    ...
    async def check_for_contradiction(
        ...
        # We only check for contradictions against other semantic facts,
        # not raw episodic logs.
        candidates = await self._vector_repo.search(
            ...
            limit=5,
            score_threshold=0.7,  # Only highly similar facts are worth checking
        )
        ...
        for point in candidates:
            ...
            # Check if hypothesis (new fact) contradicts premise (existing fact)
    ...
    async def _resolve_contradiction(
        ...
        if policy == "most_recent_wins":
            ...
            winning_fact_id = new_fact_id
        
        # If 'manual_review', we just log it and do nothing. Both remain active.

        # Log the event
        return await self._log_repo.log_contradiction(
            tenant_id=tenant.id,
            new_fact_id=new_fact_id,
