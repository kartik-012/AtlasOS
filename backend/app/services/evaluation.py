...
from app.repositories.evaluation import EvaluationMetricRepository, EvaluationRunRepository
...
class EvaluationService:
    ...
    async def trigger_evaluation(
        ...
        run = await self._run_repo.create_run(
            tenant_id=tenant_id, run_type=run_type, triggered_by=triggered_by, config=config
        )
    ...
    async def get_run_detail(self, run_id: uuid.UUID) -> tuple[EvaluationRun | None, list]:  # type: ignore
